...
const secondVideoManifest =
  videoManifest["previews/assets/vantage-h1-second-screen-final-4k.mp4"];
const hasLocalSecondVideo = fs.existsSync(secondVideoPath);
...
assert.ok(
  hasLocalSecondVideo ||
    (
      secondVideoManifest?.url?.startsWith("https://") &&
      Number(secondVideoManifest?.bytes) >= 14_000_000
    ),
  "the supplied second-screen film must be available locally or through the production video manifest",
);
assert.ok(
...
if (hasLocalSecondVideo) {
  ...
  assert.match(
    secondVideoManifest.url,
    /\/vantage-h1-second-screen-final-4k\.mp4$/,
    "the production video manifest must point at the approved second-screen film",
  );
  assert.equal(
    secondVideoManifest.bytes,
    14_320_212,
    "the production video manifest must retain the approved second-screen film",
  );
}
